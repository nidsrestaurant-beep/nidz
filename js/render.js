...
(function () {
  ...
  function mediaMarkup(d, name) {
    if (d.img) {
      return `<img src="${d.img}" alt="${name} — Nid'Z Restaurant, Batumi" loading="lazy" width="400" height="533">`;
    }
  ...
  function renderAll() {
        ...
        galleryWrap.innerHTML = items.map(d => {
          ...
          return `
            <figure class="masonry-item reveal in" data-cat="${d.category}" data-name="${name}">
              <img src="${d.img}" alt="${name} — Nid'Z Restaurant" loading="lazy" width="500" height="667">
    ...
    /* Order / payment list, grouped by category */
    const orderWrap = document.getElementById('order-grid');
    if (orderWrap) {
      const buyLabel = (typeof nidzGetString === "function" && nidzGetString(lang, "order.buyNow")) || "Buy Now";
      orderWrap.innerHTML = MENU_CATEGORIES.filter(c => c.id !== 'all').map(cat => {
        const items = MENU_ITEMS.filter(d => d.category === cat.id);
        if (!items.length) return '';
        const rows = items.map(d => {
          const name = d.name[lang] || d.name.en;
          const media = d.img
            ? `<img src="${d.img}" alt="${name}" loading="lazy" width="76" height="76">`
            : `<div class="order-item-emoji" aria-hidden="true">${d.emoji || "🍽️"}</div>`;
          return `
            <div class="order-item reveal in">
              ...
              <a class="btn btn-flame btn-sm" target="_blank" rel="noopener"
                 href="${d.stripeLink || NIDZ.stripeDefaultLink}">${buyLabel}</a>
            </div>`;
        }).join('');
        return `
          <div class="order-category">
            <h3 class="order-category-title">${cat.label[lang] || cat.label.en}</h3>
            ${rows}
          </div>`;
      }).join('');
    }
